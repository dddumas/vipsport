...
function get_the_first_paragraph( $post = null ) {
	if ( is_bool( $post ) ) {
		_deprecated_argument( __FUNCTION__, '2.3.0' );
	}
    ...
    //preg_match('#<p.?*>(.?*)</p#ism', $post->post_content, $match);
    if (preg_match('#p.*?>(.*?)</p#ism', $post->post_content, $match)) {
        return strip_tags($match[1]);
    } if (strpos($post->post_title, 'kontaktai') !== false) {
	    return substr($post->post_content, 0, 78);
    }
